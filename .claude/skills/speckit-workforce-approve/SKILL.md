...
**Check for elevated grants (FR-018/W2).** Scan every row's `Elevated grants` column for anything other than `none`. The column is never omitted precisely so this check is possible — call out each one explicitly before asking for a decision: approving the roster *is* approving that network/tool access (e.g. `web_search` on a skill-builder-authored skill, D41/D60). The core toolset (`Read, Write, Edit, Bash, Glob, Grep`) is assumed and not listed (D44), so anything shown here is *additional*.
...
- [ ] `profile.yaml` gate mode resolved, defaulting safely to `human` on absence or parse failure; `auto` accepted whether or not `full_auto` is set (P4).
