...
- [ ] T001 Scaffold `extensions/council/` (dir tree) + `install.sh` + `uninstall.sh`, mirroring `extensions/graphify/` — **simpler**: council registers no `before_*` hooks, so drop the PyYAML hook-merge; the installer only copies `extension/` and `skills/*` and (optionally) adds `council` to `.specify/extensions.yml` `installed:`.
