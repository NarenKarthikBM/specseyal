...
- Q: Council behavior when no graphify graph exists? → A: **Degrade, don't block** — the council runs deck-only and members record that graph grounding was unavailable. Grounding (D10) enhances review quality but is not a precondition for reviewing a plan.
...
- **No graphify graph present** — the council still runs deck-only, and members flag that graph-grounding was unavailable, rather than blocking. *(Ratified at clarification.)* Hard-requiring a graph would make the council brittle in ungraphed repos, and the graph is an enhancement to review quality, not a precondition for review.
...
- **A severe `/speckit-analyze` finding reopens the defended plan** — tiered review: delta by default (the council sees only the plan diff + the finding), full rerun only if the patch changes the chosen approach or architecture; the tier is proposed at triage and is human-overridable (D14). *(The `/speckit-analyze` integration lands in a later milestone; the council supports the reopen tier and its `## Reopen` record section from v1.)*
...
- **FR-005**: Council members MUST have read access to `spec.md` and an on-demand **graphify query tool** for codebase grounding (D10). When no graph is available the review proceeds deck-only and records that grounding was unavailable (see Edge Cases).
...
- **FR-017**: The system MUST support reopened-plan review (D14): when a severe `/speckit-analyze` finding reopens the plan, a `## Reopen` section is appended to `decision-record.md` with a tier (`delta` default, `full` if approach/architecture changed), proposed at triage and human-overridable.
...
- **SC-002**: **Council token spend per feature is measured** — `council_spend = phase_spend(council) + phase_spend(deck-prep)` computed from `traces.jsonl` exactly as `trace-schema.md` §5 defines it. This is the first observability datapoint and the number the M1 cost-risk decision is made against.
...
- **SC-005**: The main thread never ingests member opinions — a search for the `opinions/` path anywhere under the feature directory outside `council/` returns nothing (mechanically checkable; context hygiene).
...
*Ratified decisions and M0 contracts treated as givens for this dogfood build.*

- **Billing (D28)**: subscription-only; `ANTHROPIC_API_KEY` stays unset. No external API keys, ever.
- **Provider (D12)**: Claude-only member bench in v1 (mixed Opus/Sonnet per D18). Multi-provider is later, following Spec Kit's agent-agnostic pattern; the thin member interface (FR-003) is the one v1 concession to that future.
- **Council composition — member count**: **5** members + 1 chairman for v1 (ratified at clarification — richer diversity, classic llm-council size; the M1 risk note flags member count as the first lever to trim if SC-002 shows the run is heavy).
- **Member differentiation**: v1 members are **general-purpose** reviewers with varied prompts (D18) — faithful llm-council, not yet the v2 role-specialized critics (architect/security/cost/testability/delivery-risk). The exact prompt-variation mechanism is a `/speckit-plan` decision, not a spec commitment.
...
- **Model policy (D18)**: the role→model map in FR-016 is global policy, not per-feature; a different model for a role is a D18 amendment, not a profile edit.
- **Scope (docs/10 §8 non-goals)**: the council reviews the **plan only**, not implementation output; there is no GUI (the gate is a CLI interaction until the platform layer); there is no automated cost budgeting for council runs — cost is measured, not governed (D6).
- **Contract conformance**: every artifact conforms to the M0-blessed contracts — `artifact-layout.md`, `decision-record.md`, `trace-schema.md`, `profile-schema.md`.
- **Dogfooding boundary**: the extension is **built** in M1 but its **first live exercise** is M2's plan. For M1's own plan the human acts as the council (a bootstrap; human gate only, D9) — there is no council yet to review the council's own plan.
- **Prerequisite**: `/speckit-plan` has produced a `plan.md`, and (for FR-005 receipts-checking) a graphify graph exists for the repo; absence of the graph degrades gracefully (see Edge Cases), it does not block.
