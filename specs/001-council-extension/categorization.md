...
| T014 `/speckit-council-triage` skill | `endpoint` | `ai-agents` | false | `cli-command`, `triage`, `decision-record` |
| T015 triage provenance stub | `scaffold` | `devtools-cli` | false | `provenance`, `command-stub` |
| T016 `/speckit-council-approve` skill | `endpoint` | `ai-agents` | false | `cli-command`, `human-gate` |
...
2. **Prompt artifacts absorbed by the hybrid, not by a new type.** Member/chairman prompts and deck/suggestions templates are `*.md` → mechanically `docs` (§2 rule), with the real signal carried by `specialization: ai-agents` + tags (`prompt`, `reviewer`, `lens`). This is the D16 hybrid working as designed — the fixed core stays cheap/mechanical; nuance rides the free axis. No case for a bespoke `prompt` type.
3. **`endpoint` ⊃ CLI command, with a service fusion.** The three command skills are CLI commands (`endpoint`, §2) whose bodies are orchestration logic (service-flavored). v0 keeps them `endpoint` (deliverable = the invokable command); the fusion is noted, not resolved — a candidate refinement for the §8 review.
