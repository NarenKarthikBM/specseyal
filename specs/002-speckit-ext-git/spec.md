...
> **Positions taken.** The description named four contested points and required the spec to commit to a stance rather than defer them. Each is resolved in Functional Requirements with its rationale and rejected alternative: **branch birth moment** (FR-001), **cleanup/merge policy** (FR-011), **gate-approval↔commit-SHA binding** (FR-008/FR-009), **commit granularity** (FR-005/FR-006). Two of these refined an existing contract or decision and were **ratified at spec review (D51)**: FR-001's ordering change is applied to `artifact-layout.md` §2 (branch co-incident with `specify`); FR-011's cleanup policy was then **amended again at triage (D52): the completion anchor is a mandatory `complete/<spec-id>` tag, with `ff` permitted** — superseding D51's `--no-ff` clause only.
...
### Triage notes — Round 1 (2026-07-09, R1-S28)

- **`002`'s own `## Human Gate` is grandfathered as unbound**: it is recorded by `speckit-council-approve` *before* this very feature adds the SHA-record call (FR-008), so — like M1's ff-merge — the binding requirement does not apply retroactively to `002`'s own council gate; the first bound gate is a *later* feature's.
- **D51 timing wording disambiguated**: `after_specify` (FR-001/D-R1) fires at the **end** of the `specify` phase (after spec drafting, up-to-3 checklist iterations, and any clarify Q&A) — not literally "at ID-assignment time." "Co-incident with `specify`" (`artifact-layout.md` §2/§3, D51) means *within the specify phase, before `spec.md` is committed*, which the end-of-phase hook satisfies.
...
- **FR-001** *(branch birth moment — position taken)*: The extension MUST create the feature branch during the **`specify`** phase — as soon as the spec ID (the `specs/NNN-slug/` directory) is assigned and **before `spec.md` is committed** — so that every feature artifact, `spec.md` included, is committed on the feature branch and the base branch accrues no per-feature spec churn. *Rationale:* this matches M0/M1 manual practice (branch first) and stock spec-kit's `before_specify` hook, and satisfies D25's "branch-before-plan" a fortiori. *Rejected alternative:* a separate branch step after `clarify` (the literal ordering in `artifact-layout.md` §2) would force `spec.md`/`clarify` onto the base branch and then need a retro-commit. **Ratified at spec review (D51): `artifact-layout.md` §2 is amended so `branch` is co-incident with `specify` (context-in = the assigned spec ID), resolving the prior spec-vs-contract ordering mismatch.**
- **FR-002**: The feature branch name MUST equal the spec ID exactly (`^[0-9]{3}-[a-z0-9]+(-[a-z0-9]+)*$`, **or the `YYYYMMDD-HHMMSS-slug` form when `feature_numbering: "timestamp"` is configured** — R1-S25), identical to the feature directory name under `specs/` (D25). The branch is the sole phase state that lives outside `specs/` (D32).
...
- **FR-008** *(gate↔SHA binding — amended at triage, R1-S09/S20 + owner ruling)*: At each gate approval, the extension MUST record the immutable commit SHA of every approved artifact into a **git-ext-owned bindings artifact `specs/NNN-feature/gates.yml`** — written by the git ext as a side effect of a call the gate command already makes — and the gate section (`## Human Gate` / `## Workforce Gate`, §8) carries a **one-line reference** to it. This keeps principle I clean (no gate command's artifact is co-written by a second writer; it dissolves the D-R3 supply/record seam). The council gate binds `plan.md @ <sha>`; the workforce gate binds `tasks.md @ <sha>` + `assignment.md @ <sha>`. `gates.yml` is a durable **bindings record** under `specs/NNN/`, not phase state — phase state stays inferred from the gate section's existence (D32).
- **FR-009**: Because the approved SHA is recorded, the extension MUST **detect a stale approval** — an approved artifact whose current SHA differs from the recorded approval SHA (it was edited after approval) — surface the mismatch, and **hard-block** the phase gated by that approval: the stale approval does **not** authorize downstream work until the gate is re-run on the new content (clarified 2026-07-09; a warn-and-override was rejected as defeating the binding). **Re-approval follows the artifact's gate type (D51):** a **council**-approved artifact routes through D14's reopen tiers — `delta` by default, `full` if the approach changed — via the manual reopen interface (`001`-FR-017); a **workforce**-gate artifact simply re-runs the workforce gate. **Detection MUST be working-tree-aware (R1-S05):** a *dirty* working tree on the approved artifact (an uncommitted hand-edit) is stale — comparing committed-HEAD SHAs alone would let it sail through. **`verify-gate` MUST fail closed (R1-S10):** an unparseable-but-present binding, or a gate-format-version mismatch, is treated as stale (blocks), never fail-open. *(This is the reason FR-008's binding is worth recording.)*
...
- **FR-011** *(cleanup/merge policy — amended at spec review then D52)*: On completion, an **explicit, engineer-invoked** cleanup step (a dedicated pipeline command — never automatic, since retiring a branch is consequential; clarified 2026-07-09) MUST (a) integrate the feature into the base branch **preserving every phase-tagged commit individually** (no squash, no phase-collapsing rewrite) — **fast-forward is permitted**; a merge-commit is used only when the base has diverged; (b) create a **mandatory annotated tag `complete/<spec-id>`** at the integration commit — the feature's **completion anchor**: immutable, the binding target for D19 phase-completion events / the M5 manager, and enumerable via `git for-each-ref refs/tags/complete/*` independent of merge topology; then (c) **retire (delete) the feature branch ref** while its commits and `specs/NNN/` tree remain reachable on the base branch. A textual merge conflict **aborts and is surfaced for manual resolution** — mechanical git never guesses (no silent loss). *Position (D52, superseding D51's merge clause only):* the anchor is the **tag**, not the merge topology, so `--no-ff` is no longer required. D51's branch-at-`specify` (FR-001) and FR-008/009 reopen routing **stand unchanged**; M1's ff-grandfather is now **moot** (kept as history).
...
- **FR-015** *(the **wave-worktree spike** — not committed scope, R1-S24)*: The build MUST include a **timeboxed spike** — named the **wave-worktree spike**, distinct from `speckit-implement-parallel`'s pre-existing per-*story* `Agent isolation: worktree` guardrail — on git-worktree-per-implementation-*wave* (I-4), executed as its own experiment with a recorded outcome (finding + adopt-later/abandon recommendation) whether it succeeds or is abandoned. No FR-001…FR-014 behavior may depend on the spike's outcome (D25).
...
- **SC-002**: A feature's branch name equals its spec ID, and the base branch carries **no** per-feature spec/plan artifacts before integration (branch-first isolation, FR-001) — established by an **existence proof** on ≥1 real run (a single run cannot certify "100%", R1-S29).
...
- **SC-004**: A gate approval records the SHA of each approved artifact (in `gates.yml`), and an injected post-approval edit — **committed or uncommitted** — is detected and blocks the dependent phase until re-approval (FR-008/FR-009); shown by an **existence proof**, not a "100%" a single run can't certify (R1-S29/S05).
- **SC-005**: On completion, every phase-tagged commit remains **individually reachable** from the base tip — verified **per-SHA via `git merge-base --is-ancestor`** (not a commit count) — the **`complete/<spec-id>` tag exists** (the anchor, FR-011/R1-S27), and the feature branch ref is removed (R1-S18).
...
- **SC-007**: The extension adds **zero** AI/token cost — satisfied **by construction** (it runs as synchronous hooks, so there is no session to trace), not by a `traces.jsonl` grep that would pass vacuously (FR-007, R1-S18).
- **SC-008**: The wave-worktree spike produces a **recorded outcome + recommendation within its timebox**, regardless of result; firewalling is proven by **removing the spike and re-running Scenarios 1–4 green** (not by grepping for a `worktree` reference) — FR-015, R1-S18/S24.
