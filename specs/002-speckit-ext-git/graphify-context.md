...
- `extensions/graphify/install.sh` — the **hook-registering** install variant: graphify's installer merges its `before_*` hooks into `.specify/extensions.yml`. 002 needs this variant (it registers `after_*` commit hooks + `before_tasks`/`before_implement` verify-gate hooks — **not** a `before_specify` hook; branch birth is folded into `after_specify`, D-R1/R1-S16), where council's installer did **not** (council is command-only). 002 = council's packaging + graphify's hook-merge.
