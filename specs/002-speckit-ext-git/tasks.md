...
- [ ] T022 Wire the **`before_specify` drift lint into the D50 conformance checker** (**R1-S29**) — mechanically catch the `before_specify`-style assertion-drift class, partially compensating the prose-level enforcement gap (R1-S08 / D53). `files=extensions/git/test/run.sh` (or the D50 checker location, if it exists) `deps=T020` `mutates=extensions/git/test/run.sh`
...
- Wave 6 [parallel] : T008 [US1], T011 [US3], T015 [US3], T016 [US4], T018 [US5]   # story kickoffs, disjoint
...
- Wave 10 [serial]  : T019                              # graphify-context manifest correction (shared file)
- Wave 11 [serial]  : T020                              # before_specify assertion fix (shares graphify-context.md with T019)
- Wave 12 [parallel]: T021, T022                        # test harness + drift lint
- Wave 13 [serial]  : T023                              # quickstart SC validation run (deps ~all)
...
- T018  files=specs/002-speckit-ext-git/implement.log.md,docs/90-DECISIONS-AND-IDEAS.md   deps=-   mutates=docs/90-DECISIONS-AND-IDEAS.md
...
- T022  files=extensions/git/test/run.sh                               deps=T020        mutates=extensions/git/test/run.sh
...
- `specs/002-speckit-ext-git/graphify-context.md` — touched by **T019** (manifest correction) and **T020** (before_specify fix) → pinned to consecutive serial waves (10, 11).
- `extensions/git/test/run.sh` — authored by **T021**, extended by **T022** → serialized (T022 after T021).
