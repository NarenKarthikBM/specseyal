...
- [X] T020 [US2] **Reinstall git-ext** and add the **reinstall-survival regression** to `extensions/git/test/run.sh`: assert **both** gate wirings fire after reinstall — `after_council_approve` (council) **and** `after_workforce_approve` (workforce → `gates.yml` gets `tasks.md`+`assignment.md@sha`) (S02/S07/S17). (depends on T018, T019)
