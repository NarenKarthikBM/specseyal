...
>
> The categorizer and the assembler **PROPOSE**; the human **signs** at the workforce gate (D9). Nothing here decides autonomously unless a feature's `profile.yaml` sets `gates.workforce.mode: auto` (valid **standalone** per `profile-schema.md` P4 — see FR-020, amended by D67 verdict 12 — except that any elevated grant in the roster forces the gate back to a human).
...
- **FR-020**: The gate section MUST record the human's decision (`approved` | `approved-with-notes` | `rejected`); `approved`/`approved-with-notes` unlocks `/speckit-implement-parallel`, a `rejected` returns the roster for reassignment (artifact-layout §8 W3). Under `gates.workforce.mode: auto` the assigner writes the section itself — **valid standalone, NOT only within `full_auto`** (amended by **D67 verdict 12**, resolving **I-19**: `profile-schema.md` P4 governs — the workforce gate is an economic guard, so the standalone-auto reading, which the shipped skill already followed, is ratified). **Carve-out — the D67 grant tripwire:** if the assembled roster carries ANY elevated grant, the gate is forced to a **human** regardless of the profile; `auto` never approves a grant-bearing roster, because approving it is approving network/tool access (D41/A-2). *(This supersedes the FR's original "only within `full_auto`" clause; the spec↔`profile-schema.md`-P4 drift I-19 flagged is resolved in P4's favour, plus the tripwire.)*
...
- **Autonomy** (D9, `profile-schema.md`): the workforce gate is the second gate-capable checkpoint; the categorizer and assembler PROPOSE, the human signs — unless `gates.workforce.mode: auto` (valid standalone per P4, FR-020/D67 verdict 12; overridden back to human by the D67 grant tripwire when the roster carries any elevated grant).
