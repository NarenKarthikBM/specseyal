...
The categorizer's sole output; the `workforce` extension's `/speckit-categorize` command owns the file, writes nothing else (D37).
...
- **Key fields:** `specseyal.kind: skill`, `id: skl_…`, `version` semver, `origin ∈ {seed,generated,promoted}`, `taxonomy.tags: [..]` (non-empty, **no type/specialization**), `grants: []`-or-elevated (D41 — every **seed** skill is `grants: []`; the **skill-builder's own module declares `grants: [web_search]`**, D60, the system's first elevated grant), `provenance.source_feature` (non-null iff generated/promoted), `provenance.stale_risk` (S17 — set when authored post-cutoff without searching), `stats` (derived cache), **no `model`**.
...
The roster table is **written by `assemble.py` itself** (S08 — a tool-permission fact, so the session cannot re-rank or hand-transcribe), which **total-orders every set-typed intermediate (the grant union above all) before serializing** (S01) and stamps a **library-snapshot content-hash** into the section (S18), so a gap-free roster is byte-reproducible and checkable in one line.
...
| Elevated grants | `⋃ skills.grants` | **never omitted** (W2/FR-018); `none` when empty; core toolset never listed (D44); `web_search` on **builder-path** rows (D60) |

**Gate record:** `## Workforce Gate` with reviewer · `decision ∈ {approved, approved-with-notes, rejected}` · reviewed. `approved`/`approved-with-notes` unlocks implement; `rejected` returns for reassignment (W3/FR-020). Present iff `gates.workforce.mode: human` (W4); the human signature is recorded by **`/speckit-workforce-approve`** (S13), which fires `after_workforce_approve` → git-ext's `on-gate-approve.sh workforce` to bind `tasks.md`+`assignment.md@sha` into `gates.yml` (S02). Under `auto` (only within `full_auto`) the assigner writes the section and the same gate-write fires (FR-020/W4).
