...
> **✅ APPROVED with adjudications — 2026-07-12 (D68).** Spec review ratified the three positions **as taken** (doc-only `executed: none` boundary; completion report = validatable D19 `artifact.body` with the normative-core-vs-dogfood-overlay split; both gates `human` to exercise the first machine-bound workforce gate). **I-17 ruled (a) — in-scope:** M4 absorbs the git-ext workforce-freshness fix as the distinct **Prerequisite** FR-group (FR-014–017) + **SC-010**, owned-source + survival-tested, sequenced wave-1/pre-wave, its design council-reviewed as gate-integrity. The testing seam and completion-authorship topology stay **deferred to the plan** (non-binding leans on record). See `docs/90` **D68**.
...
**Prerequisite — workforce freshness under implement** (I-17, ruled in-scope, **D68** — a **distinct second concern** the council reviews as such, **not** part of the testing agent)

- **FR-014**: M4 MUST include a fix to the git extension's **I-17 workforce-freshness defect** so that the per-wave `verify-gate workforce` freshness check **passes for waves 2+** during an implement run **without hand assistance**. *(The defect: `implement-parallel` marks tasks `[X]` and commits per wave; the workforce binding covers `tasks.md`; `verify-gate` is working-tree- and SHA-aware, so the first `[X]` mark/commit stales the binding and hard-blocks wave 2+ — a **general** defect that bites every feature, surfaced now because M4 is the first feature to run its own implement under the live workforce machinery without a grandfather.)*
- **FR-015**: The fix MUST be an **owned-source edit in the git extension** (artifact-layout §9 / D57 S2 — never an installed-copy edit) and MUST carry **reinstall + reinstall-survival-regression coverage** (the S17 class; `extensions/git/test/run.sh` §3 is the model).
- **FR-016**: The fix MUST be **sequenced to land in wave 1 (or pre-wave)** of M4's own implement, so waves 2+ run under the corrected machinery — keeping "the pipeline's first fully-unassisted run" literally true through implement (SC-010).
- **FR-017**: The fix **design** is plan-level and MUST be **council-reviewed as gate-integrity** (a security-lens concern — it changes what the workforce freshness guard admits). *Non-binding lean for the plan to argue (D68): the defect is a **trust-classification error** — `[X]`-marking and per-wave commits are **machinery** acts (`commit.sh`), not tampering; a binding that **refreshes on machinery-authored wave commits** while still hard-blocking human/content edits preserves exactly what S05/FR-009 exist to catch. The plan may reject this shape with reasons.*
...
- **SC-010** (**I-17 prerequisite, D68**): During M4's **own** implement, **wave 2+ passes the `verify-gate workforce` freshness check under the live machinery with zero hand assistance** — the I-17 fix proven on the dogfood run itself, so "the first fully-unassisted run" holds through implement.
...
- **I-17 workforce-freshness fix is in-scope** (D68, ruled (a)): M4 absorbs the general git-ext `[X]`-marking-stales-the-binding defect as an owned-source git-ext edit (D57/S2), scoped as the **Prerequisite** FR-group (FR-014–017) + SC-010 — a distinct **second concern**, sequenced wave-1/pre-wave so M4's own implement runs unassisted (the exit test stays literally true).
...
## Spec-review dispositions (D68) & items deferred to the plan

- **I-17 — RULED (a), IN-SCOPE (D68).** M4 absorbs the git-ext workforce-freshness fix (the general `[X]`-marking-stales-the-binding defect). It is scoped as the **Prerequisite** FR-group (FR-014–017) + **SC-010** — a distinct **second concern** the council reviews as such — owned-source + reinstall-survival-tested (D57/S2, S17 class), sequenced **wave 1 (or pre-wave)**, its **design** council-reviewed as gate-integrity with a non-binding **trust-classification** lean (FR-017). *Rationale (D68): M4 is the first feature to run its own implement under the live workforce machinery WITHOUT the `003` grandfather (I-16 was `003`-only), so grandfathering again (option c) would undercut the exit test; fixing here also clears a defect that bites every future feature. A bootstrap-escape for I-16 may ride along — a plan call.*

- **The git-ext seam mechanism — deferred to the plan** (non-binding lean, D68): an `after_testing` hook + `testing` added to `commit.sh`'s phase enum — the `002` pattern (a hook + an owned-source enum entry). The plan takes a position; the council reviews.

- **Completion authorship topology — deferred to the plan** (non-binding lean, D68): a thin `/speckit-complete` command over orchestrator-inline authorship — because M5's `after_complete` phase-event push (D19) needs a **command boundary to hang the hook on**. The phase still runs in `main` either way (artifact-layout §2). The plan takes a position; the council reviews.
