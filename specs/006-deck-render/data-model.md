...
| O5 | **A stale render is never presented as current** (US3-3). A failed render does not leave a previous run's file to be mistaken for fresh output: the target is removed before the attempt, so a failure leaves *no* file rather than an old one. The stamp is the second line of defense (SC-007). |
