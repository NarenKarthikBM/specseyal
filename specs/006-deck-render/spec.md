...
- Q: Where does the rendered output live, and is it committed to the repo? → A: **A gitignored, derived build product** — written under a `renders/` path, never committed, regenerated on demand from the markdown. Stakeholder sharing (the I-6 lineage) happens out-of-band with the local file. *Rationale: the renderer is deterministic and model-free (FR-011), so regeneration is free; committing a binary would put a stale receipt in the history where a reader could mistake it for the record — the hazard D59 named about the graph — and would leave no pptx in git to confuse with the artifact of record.* (Applied to FR-014, SC-005, Key Entities, Constraints.)
- Q: What triggers the render, given that no existing hook fires between deck-prep and the human gate? → A: **A standalone, on-demand command** the human runs before/at the gate. *Rationale: the registered hooks cannot serve the gate — `after_plan` fires before the deck exists, and `after_council_approve` fires after the human has already signed. Creating an `after_council` seam would require a source edit into the council extension, contradicting FR-012 (D57) and colliding with `005`'s concurrent rewrite. The on-demand command needs **no** council coupling, and it makes staleness structurally impossible: it renders whatever markdown exists at the moment it runs.* (Applied to FR-008, Key Entities, Edge Cases.)
- Q: How is "content-faithful" (FR-002) mechanically verified? → A: **Bidirectional text-set containment on a committed fixture deck** — extract the render's text, then assert (1) every heading and text block in the source markdown is present (nothing dropped) **and** (2) the render introduces no text beyond the source plus a known allowlist (the derived-render stamp + structural chrome) — nothing invented. *Rationale: a one-way check catches dropped content but not invented content, and invented content is the dangerous half — that is the render saying something the council never reviewed.* (Applied to FR-002, SC-003.)
- Q: What is the renderer's dependency posture — hard install requirement, or optional? → A: **Optional / lazy.** Install never requires a presentation toolchain; the render command checks for it at invocation and degrades-and-discloses if it is absent. *Rationale: the feature is default-off (FR-006), so making every SpecSeyal install acquire a toolchain for a feature almost nobody has enabled inverts the cost/benefit, and a hard dependency fails installs on constrained hosts. The lazy check **is** the degrade path FR-009/FR-010 already require, so it costs no new machinery.* (Applied to FR-015, SC-004.)
- Q: Does `deck_render: none` forbid the render command, or only mean "don't render by default"? → A: **Default selection, not a hard gate.** An argument-less invocation renders what the profile selects (`none` ⇒ nothing); an invocation that explicitly names a deck renders it regardless of profile. *Rationale: a hard gate makes **SC-009 unsatisfiable by construction** — `006`'s own profile is necessarily `none` (the renderer does not exist when its own council convenes), yet its exit test must render its own deck. It would also kill I-6 ad-hoc reuse (rendering a past feature's overview for a demo without rewriting its settled profile). The boundary is untouched: an explicitly-rendered deck is still derived, un-bound, un-traced, gitignored.* (Applied to FR-016, SC-001, SC-009, Constraints.)
...
- **The deck is regenerated on a revision round** (D38 — `defense-deck/` is overwritten in place when a blocking suggestion forces a plan revision). Because the render is **on-demand** (FR-008), the next invocation simply renders the new deck — there is no re-derivation bookkeeping to get wrong. A render left on disk from *before* the revision is **stale**, and its stamp is what reveals it: stamped source-SHA ≠ current source-SHA (FR-003, SC-007). A stale render must never be presented at the gate as current.
...
- **FR-002**: The renderer MUST be **content-faithful**, in the falsifiable sense of **bidirectional text containment** (clarified 2026-07-14): (a) **nothing dropped** — every heading and text block in the source markdown is present in the render's extractable text; and (b) **nothing invented** — the render's text introduces nothing beyond the source, save a known allowlist (the derived-render stamp and structural chrome). **The renderer transforms; it never authors.** *(Direction (b) is the load-bearing half: a render that could add or re-word content could make the pptx say what the reviewed markdown does not — the precise failure FR-001 exists to prevent.)*
...
- **FR-008**: The render MUST be triggered by a **standalone, on-demand command**, invocable by the human before or at the council gate, and it MUST render the deck markdown **as it exists at invocation time** — so the render always corresponds to what the human is about to read. It MUST NOT be wired into the council phase's own execution (no new hook point inside the council extension — see FR-012). *(Clarified 2026-07-14. The registered hook set cannot serve the gate: `after_plan` fires before the deck exists, `after_council_approve` fires after the human has signed. On-demand invocation also makes staleness structurally impossible — there is no window in which a render silently ages relative to the deck the human is reading, because the human re-runs it. A revision round (D38) therefore needs no re-derivation bookkeeping: the next invocation renders the new deck.)*
...
- **FR-010**: A render failure MUST be **disclosed** to the human who invoked the render — never silent — and the disclosure MUST report a **per-deck outcome**, so that a partial failure under `deck_render: both` (one deck rendered, one failed) cannot be misread as success. The disclosure MUST state that the markdown is unaffected and remains the artifact of record. *(The FR-019 reduced-grounding lineage, D46(4): a degraded state is surfaced where the decision is made, never presented as whole. Because the render is on-demand (FR-008), the human **is** the invoker — the disclosure reaches them directly, and no council-owned artifact is written to, preserving `artifact-layout.md` §6 ownership.)*
...
**Added at clarify (2026-07-14).**

- **FR-014**: A rendered deck MUST be a **derived build product, not a committed artifact**: written under a gitignored path, never tracked by git, and regenerable from its source markdown at any time. It MUST NOT be written into the council-owned `council/` subtree (`artifact-layout.md` §6 — no writer touches another's artifact). *(Committing a render would place a stale binary receipt in the history where it could be mistaken for the record — the D59 hazard — and the render is free to regenerate, so committing buys nothing for what it costs.)*
- **FR-015**: Any presentation toolchain the renderer needs MUST be an **optional, lazily-checked dependency**: installing the extension MUST NOT require it, and MUST NOT fail on a host that lacks it. The command checks for it at invocation and, when it is absent, degrades and discloses per FR-009/FR-010. *(The feature is default-off — making every install acquire a toolchain for a feature almost nobody enables inverts the cost/benefit.)*
- **FR-016**: `deck_render` is the **default selection, not a hard gate**. An invocation with no explicit deck argument renders exactly what the profile selects (`none` ⇒ nothing). An invocation that **explicitly names a deck** renders it regardless of the profile — config declares what the pipeline does on its own; an explicit human invocation is an explicit act. **The boundary is unchanged either way**: an explicitly-rendered deck is still derived, still un-bound, still un-traced, still gitignored (FR-001, FR-014). *(This is what makes SC-009 satisfiable — `006`'s own profile is necessarily `none`, yet its exit test renders its own deck — and what keeps the I-6 door open: a past feature's overview can be rendered for a demo without rewriting its long-settled profile.)*
...
- **Rendered deck (presentation file)** — a **derived render** of one markdown deck, and a **gitignored build product** (never committed, regenerable at will — clarified 2026-07-14). Optional, default-absent, per-deck. Never reviewed, never gate-bound, never traced, never an input to any phase. Carries a derived-render stamp naming its source path and source SHA.
...
- **The render command** — a standalone, on-demand command the human invokes before/at the council gate (clarified 2026-07-14). It reads the feature's `deck_render` selection and the current deck markdown, and writes the derived render. It is **not** part of the council phase's execution and adds no hook point inside the council extension.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001** (**the default path is untouched**): With `deck_render` absent or `none` and no explicit render invocation, a full council phase produces **zero** rendered files, **zero** new trace records, and a `council/` subtree byte-identical to the pre-006 pipeline's (FR-007, FR-013, FR-016).
...
- **SC-003** (**fidelity, both directions**): Against a **committed fixture deck**, the render's extractable text satisfies **bidirectional containment** — **100%** of the source markdown's headings and text blocks are present (nothing dropped), and **zero** text appears that is not in the source or on the stamp/chrome allowlist (nothing invented). Both directions are asserted mechanically; neither is eyeballed (FR-002).
- **SC-004** (**degrade, never halt**): Under a forced render failure — including the **toolchain-absent** case (FR-015), which is the likeliest one in the field — the council phase completes, the gate is reachable and approvable, **every** `.md` under `council/` is byte-identical, and a per-deck failure notice reaches the human (FR-009 **and** FR-010 together — the phase survives *and* the human is told, per deck).
- **SC-005** (**the boundary holds mechanically**): Across a rendered run, no rendered file appears in `gates.yml`, in any `traces.jsonl` record, in any council session's context-in, or in **git's tracked file set** (`git ls-files` returns no rendered output). *(The falsifiable form of FR-001 + FR-014 — grep-able, not asserted.)*
...
- **SC-009** (**006 exit, dogfood**): After implement, an **explicit** render invocation against **006's own** committed `council/defense-deck/overview.md` produces a valid, stamped, content-faithful rendered deck. *(006's own council necessarily runs with `deck_render: none` — the renderer does not exist when its own plan is defended — so the exit test relies on FR-016's explicit-invocation path, not the profile. See Constraints. `007` is the first feature that can enable the flag at council time and have it fire by default.)*
...
- **`profile.yaml` — `deck_render: none` for `006` itself** (a bootstrap fact, the D46(2) class): the renderer does not exist when `006`'s own council convenes, so `006`'s council necessarily runs unrendered. This is a bootstrap fact, not a conformance gap. `007-oss-docs` is the first feature that can set the flag at council time **and have it fire by default**. `006`'s own dogfood exit test (SC-009) therefore renders via **FR-016's explicit-invocation path**, not via its profile — the profile keeps telling the truth about what its council actually ran with. *(The profile is authored at plan time — the `004` precedent — with these choices recorded here as Constraints.)*
