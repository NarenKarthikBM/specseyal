...
<!-- DELIBERATELY BROKEN FIXTURE (R1-S03 negative control): byte-identical to
     testing.golden.md except the FR-011 row is missing entirely from the
     Coverage map below -- an id in spec.md with NO row at all, the exact gap
     shape the coverage validator must fail loudly on. Not to be confused
     with SC-009's row further down, which IS present and honestly marked
     status: GAP -- that is a valid, surfaced gap; this file tests the other
     failure mode, an id missing a row altogether. -->
